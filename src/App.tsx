...
import {
  ArrowRight, ArrowLeft, Play, RefreshCw, User, Shield, FileText,
  Circle, Box, Menu, X, Globe, ExternalLink,
  Timer, PlayCircle, PauseCircle, RotateCcw, Layout, Users, BookOpen, Utensils,
  List, ClipboardList, Cpu, ShoppingCart, Globe2, Star, Heart
} from 'lucide-react';

// --- CONFIGURACIÓN ---
const FINAL_WEB_URL = "https://e1-yantaconseso.netlify.app";

// --- IMPORTACIÓN DE IMÁGENES ---
// Gestión Global
import imgGlobalDiag from './assets/diagrama-clases-gestion-global.png';
import imgGlobalComp from './assets/componentes-gestion-global.png';
import imgGlobalFile from './assets/archivo-gestion-global.png';
// General Diagram
import imgGeneralDiag from './assets/general_diagram.png';
// Video
import videoInicio from './assets/inicio.mp4';
import videoFin from './assets/final.mp4';

// Administración de Recetas
import imgRecetaDiag from './assets/diagrama-clases-administracion-recetas.png';
import imgRecetaComp from './assets/Componentes-administracion-recetas.png';
import imgRecetaFile from './assets/archivo-administracion-recetas.png';

// Compartir Recetas
import imgShareDiag from './assets/diagrama-clases-compartir.png';
import imgShareComp from './assets/componentes-compartir-recetas.png';
import imgShareFile from './assets/archivo-compartir-recetas.png';

// Favoritos
import imgFavDiag from './assets/diagrama-clases-favoritos.png';
import imgFavComp from './assets/componentes-favoritos.png';
import imgFavFile from './assets/archivo-favoritos.png';

// Buscar y Descubrir
import imgSearchDiag from './assets/diagrama-clases-buscar.png';
import imgSearchComp from './assets/componentes-buscar-descubrir.png';
import imgSearchFile from './assets/archivo-buscar-y-descubir.png';

// Planificación Semanal
import imgWeeklyDiag from './assets/diagrama-clases-planificacion.png';
import imgWeeklyComp from './assets/componentes-planificacion.png';
import imgWeeklyFile from './assets/archivo-planificacion-semanal.png';

// Crear Lista de la Compra
import imgShopDiag from './assets/diagrama-clases-lista-compra.png';
import imgShopComp from './assets/componentes-lista-compra.png';
import imgShopFile from './assets/archivo-crear-lista-compra.png';

// Moderación
import imgModDiag from './assets/diagrama-clases-moderacion.png';
import imgModComp from './assets/componentes-moderacion.png';
import imgModFile from './assets/archivo-moderacion.png';

// Mapeo de imágenes por diagrama
const diagramImages: Record<string, string[]> = {
  'global-user-mgmt': [imgGlobalDiag, imgGlobalComp, imgGlobalFile],
  'recipe-mgmt': [imgRecetaDiag, imgRecetaComp, imgRecetaFile],
  'share-recipe': [imgShareDiag, imgShareComp, imgShareFile],
  'favorites-ratings': [imgFavDiag, imgFavComp, imgFavFile],
  'search-discover': [imgSearchDiag, imgSearchComp, imgSearchFile],
  'weekly-planning': [imgWeeklyDiag, imgWeeklyComp, imgWeeklyFile],
  'shopping-list': [imgShopDiag, imgShopComp, imgShopFile],
  'main-sequence': [imgModDiag, imgModComp, imgModFile],
};
...
const Lifeline: React.FC<LifelineProps> = ({ x, height, label, subLabel, icon: Icon, color = "text-blue-400", type = "actor" }) => (
...
const Message: React.FC<MessageProps> = ({ fromX, toX, y, label, isReturn = false, isSelf = false, show }) => {
  if (!show) return null;
...
const ActivationBox: React.FC<ActivationBoxProps> = ({ x, y, height, show }) => {
  if (!show) return null;
  return (
    <rect
      x={x - 6}
...
const GroupFrame: React.FC<GroupFrameProps> = ({ x, y, width, height, type, label, show }) => {
  if (!show) return null;
...
const AltDivider: React.FC<AltDividerProps> = ({ x, y, width, label, show }) => {
  if (!show) return null;
  return (
    <g className="animate-fade-in">
...
export default function App() {
  ...
  // Derived state for diagram logic
  const hasImages = !!diagramImages[activeDiagram.id];
  const introSteps = hasImages ? 3 : 0;
  // If we are in the intro steps, the diagram step is 0 or -1 (doesn't matter as long as diagram logic handles it or is not rendered)
  // Let's make "step" 0 if globalStep < introSteps, so the diagram is ready at start state when we transition.
  const step = Math.max(0, globalStep - introSteps);

  const totalSteps = activeDiagram.steps; // This now includes the +3
  ...
  const focusPointsWeeklyPlanning = {
    ...
    6: 250, 7: 280, 8: 300, 9: 320, 10: 350, 11: 380, 12: 400,
  ...
  useEffect(() => {
    // Only autoscroll if we are past the intro images or if the diagram has no images
    if (scrollContainerRef.current && (globalStep >= introSteps || !hasImages) && step < (totalSteps - introSteps)) {
      const scrollContainer = scrollContainerRef.current;
      let currentPoints: Record<number, number>;
      if (activeDiagram.id === 'home') currentPoints = focusPointsHome;
      else if (activeDiagram.id === 'main-sequence') currentPoints = focusPointsMain;
      ...
      else if (activeDiagram.id === 'weekly-planning') currentPoints = focusPointsWeeklyPlanning;
  ...
  const nextStep = () => setGlobalStep(prev => Math.min(prev + 1, totalSteps));
  const prevStep = () => setGlobalStep(prev => Math.max(prev - 1, 0));
  const reset = () => setGlobalStep(0);

  const handleDiagramChange = (diagram: any) => {
  ...
  }, [totalSteps, step, globalStep, hasImages, introSteps]);
  ...
  const renderHome = () => {
    if (globalStep === 1) {
      return (
        <div className="w-full h-full flex flex-col items-center justify-center p-8 animate-in fade-in slide-in-from-right duration-500 bg-slate-950">
          <h2 className="text-3xl font-bold text-white mb-6 drop-shadow-md">Arquitectura General del Sistema</h2>
          <div className="relative max-w-6xl w-full h-[80vh] flex items-center justify-center bg-slate-900 rounded-xl border border-slate-700 p-4 shadow-2xl">
            <img
              src={imgGeneralDiag}
              alt="Diagrama General de Casos de Uso"
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 animate-in fade-in zoom-in duration-500 bg-slate-950">
        <div className="max-w-4xl w-full text-center">
          <h1 className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400 mb-8 drop-shadow-lg">
            Sistema de Gestión de Recetas
          </h1>
          <p className="text-slate-400 text-lg mb-12 max-w-2xl mx-auto">
            Bienvenido a la documentación interactiva de la arquitectura del sistema.
            Navegue por los diferentes módulos para visualizar los flujos de secuencia.
          </p>

          <div className="relative w-full aspect-video bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden group">
            <video
              src={videoInicio}
              controls
              autoPlay
          ...
          <div className="mt-12 flex justify-center gap-4">
            <button
              onClick={() => nextStep()}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium transition-all shadow-lg shadow-blue-900/20 flex items-center gap-2"
            >
              Ver Esquema General <ArrowRight size={18} />
            </button>
          </div>
        </div>
      </div>
  ...
  const renderAcknowledgments = () => (
    <div className="w-full h-full flex flex-col items-center justify-center p-8 animate-in fade-in zoom-in duration-500 bg-slate-950 text-center">
      <div className="max-w-4xl w-full">
        <div className="mb-12">
          <Heart size={64} className="text-pink-500 mx-auto mb-6 animate-pulse" />
          <h1 className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-purple-400 mb-6 drop-shadow-lg">
            Agradecimientos
          </h1>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto leading-relaxed">
            Queremos expresar nuestro más sincero agradecimiento a todo el equipo,
            colaboradores y mentores que hicieron posible este proyecto.
          </p>
        </div>

        <div className="relative w-full aspect-video bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden group mx-auto">
          <video
            src={videoFin}
            controls
            autoPlay
        ...
        <div className="mt-12 text-slate-500 text-sm">
          © 2024 Proyecto de Arquitectura de Software
        </div>
      </div>
    </div>
  );
  ...
  const renderIntroImages = () => {
    if (!hasImages) return null;
    const images = diagramImages[activeDiagram.id];
    // globalStep 0 -> image 0
    // globalStep 1 -> image 1
    // globalStep 2 -> image 2
    const currentImage = images[globalStep];
    const labels = ["Diagrama Completado", "Componentes de la Sección", "Estructura de Archivos"];

    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-slate-950 animate-in fade-in zoom-in duration-500">
        <h2 className="text-3xl font-bold text-white mb-6 drop-shadow-md">{labels[globalStep]}</h2>
        <div className="relative max-w-5xl w-full max-h-[80vh] flex items-center justify-center bg-slate-900 rounded-xl border border-slate-700 p-4 shadow-2xl">
          <img
            src={currentImage}
            alt={labels[globalStep]}
            className="max-w-full max-h-full object-contain rounded-lg shadow-lg"
          />
        </div>
      </div>
    ...
    return (
      <div className="min-w-[1400px] p-8 flex justify-center items-start pt-16">
        <svg ref={svgRef} width="1350" height={svgHeight} viewBox={`0 0 1350 ${svgHeight}`} className="drop-shadow-2xl font-sans">
          ...
          {/* Continuous Activation Bars */}
          <ActivationBox x={pos.user} y={100} height={1450} show={step >= 0} />
          <ActivationBox x={pos.admin} y={100} height={1450} show={step >= 0} />
    ...
    return (
      <div className="min-w-[1200px] p-8 flex justify-center items-start pt-16">
        <svg width="1200" height={svgHeight} viewBox={`0 0 1200 ${svgHeight}`} className="drop-shadow-2xl font-sans">
          ...
          {/* Continuous Activation Bars */}
          <ActivationBox x={pos.user} y={100} height={3650} show={step >= 0} />
          <ActivationBox x={pos.admin} y={100} height={3650} show={step >= 0} />
          ...
          <Message fromX={pos.panelCtrl} toX={pos.usuarios} y={240} label="2: saveUser()" show={step >= 4} />
          <ActivationBox x={pos.operaciones} y={240} height={30} show={step >= 4} />
          <Message fromX={pos.usuarios} toX={pos.operaciones} y={260} label="2.1: saveUser()" show={step >= 4} />
          ...
          <ActivationBox x={pos.operaciones} y={540} height={40} show={step >= 11} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={550} label="5: checkUser()" show={step >= 11} />
          <Message fromX={pos.operaciones} toX={pos.panelCtrl} y={570} label="5.1: boolUser()" isReturn={true} show={step >= 11} />
          ...
          <GroupFrame x={pos.admin - 40} y={1200} width={pos.usuarios - pos.admin + 80} height={2300} type="alt" label="[ClickOn_AdministracionUsers == true]" show={step >= 16} />
          ...
          <ActivationBox x={pos.operaciones} y={1770} height={30} show={step >= 21} />
          ...
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={1800} label="23.1.1: guardarCambios()" show={step >= 21} />

          {/* --- BLOQUE CONFIGURACIÓN (NUEVO) --- */}
          <AltDivider x={pos.admin - 40} y={1850} width={pos.usuarios - pos.admin + 80} label="[ClickOn_Configuracion == true]" show={step >= 22} />

          <GroupFrame x={pos.admin - 20} y={1880} width={pos.usuarios - pos.admin + 40} height={250} type="loop" label="[Hasta que operacionesPendientes == 0]" show={step >= 23} />

          <ActivationBox x={pos.panelIU} y={1920} height={150} show={step >= 23} />
          <ActivationBox x={pos.panelCtrl} y={1930} height={130} show={step >= 23} />
          <ActivationBox x={pos.operaciones} y={1950} height={80} show={step >= 23} />

          <Message fromX={pos.admin} toX={pos.panelIU} y={1940} label="24: operationUsers()" show={step >= 23} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={1960} label="24.1: operationUsers()" show={step >= 23} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={1980} label="24.1.1: operationUsers()" show={step >= 23} />
          <Message fromX={pos.operaciones} toX={pos.panelCtrl} y={2010} label="24.1.2: getOperationUser()" isReturn={true} show={step >= 23} />
          <Message fromX={pos.panelCtrl} toX={pos.panelIU} y={2030} label="24.1.2: getOperationUser()" isReturn={true} show={step >= 23} />

          <GroupFrame x={pos.admin - 20} y={2160} width={pos.usuarios - pos.admin + 40} height={200} type="loop" label="[Hasta que errorOperation == false]" show={step >= 24} />

          <ActivationBox x={pos.panelIU} y={2190} height={100} show={step >= 24} />
          <ActivationBox x={pos.panelCtrl} y={2200} height={80} show={step >= 24} />
          <ActivationBox x={pos.operaciones} y={2210} height={30} show={step >= 24} />

          <Message fromX={pos.admin} toX={pos.panelIU} y={2210} label="25: operacionCompleta()" show={step >= 24} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={2230} label="25.1: operacionCompleta()" show={step >= 24} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={2250} label="25.1.1: borrarOperacion()" show={step >= 24} />

          <ActivationBox x={pos.panelIU} y={2390} height={80} show={step >= 25} />
          <ActivationBox x={pos.panelCtrl} y={2400} height={60} show={step >= 25} />
          <ActivationBox x={pos.operaciones} y={2410} height={30} show={step >= 25} />
          <Message fromX={pos.admin} toX={pos.panelIU} y={2400} label="26: guardarCambios()" show={step >= 25} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={2420} label="26.1: guardarCambios()" show={step >= 25} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={2440} label="26.1.1: guardarCambios()" show={step >= 25} />

          {/* --- AUDITORIA (RE-INDEXED 26-29) --- */}
          <AltDivider x={pos.admin - 40} y={2670} width={pos.usuarios - pos.admin + 80} label="[ClickOn_Auditoria == true]" show={step >= 26} />

          <GroupFrame x={pos.admin - 20} y={2700} width={pos.usuarios - pos.admin + 40} height={250} type="loop" label="[Hasta que operacionesPendientes == 0]" show={step >= 27} />

          <ActivationBox x={pos.panelIU} y={2740} height={150} show={step >= 27} />
          <ActivationBox x={pos.panelCtrl} y={2750} height={130} show={step >= 27} />
          <ActivationBox x={pos.operaciones} y={2770} height={80} show={step >= 27} />

          <Message fromX={pos.admin} toX={pos.panelIU} y={2760} label="27: operationUsers()" show={step >= 27} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={2780} label="27.1: operationUsers()" show={step >= 27} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={2800} label="27.1.1: operationUsers()" show={step >= 27} />
          <Message fromX={pos.operaciones} toX={pos.panelCtrl} y={2830} label="27.1.2: getOperationUser()" isReturn={true} show={step >= 27} />
          <Message fromX={pos.panelCtrl} toX={pos.panelIU} y={2850} label="27.1.2: getOperationUser()" isReturn={true} show={step >= 27} />

          <GroupFrame x={pos.admin - 20} y={2980} width={pos.usuarios - pos.admin + 40} height={200} type="loop" label="[Hasta que errorOperation == false]" show={step >= 28} />

          <ActivationBox x={pos.panelIU} y={3010} height={100} show={step >= 28} />
          <ActivationBox x={pos.panelCtrl} y={3020} height={80} show={step >= 28} />
          <ActivationBox x={pos.operaciones} y={3030} height={30} show={step >= 28} />

          <Message fromX={pos.admin} toX={pos.panelIU} y={3030} label="28: operacionCompleta()" show={step >= 28} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={3050} label="28.1: operacionCompleta()" show={step >= 28} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={3070} label="28.1.1: borrarOperacion()" show={step >= 28} />

          <ActivationBox x={pos.panelIU} y={3210} height={80} show={step >= 29} />
          <ActivationBox x={pos.panelCtrl} y={3220} height={60} show={step >= 29} />
          <ActivationBox x={pos.operaciones} y={3230} height={30} show={step >= 29} />
          <Message fromX={pos.admin} toX={pos.panelIU} y={3220} label="29: guardarCambios()" show={step >= 29} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={3240} label="29.1: guardarCambios()" show={step >= 29} />
          <Message fromX={pos.panelCtrl} toX={pos.operaciones} y={3260} label="29.1.1: guardarCambios()" show={step >= 29} />

          {/* SALIDA MEJORADA (PASOS 30-32) - AHORA DENTRO DEL ALT */}
          <AltDivider x={pos.admin - 40} y={3300} width={pos.usuarios - pos.admin + 80} label="[ClickOn_Salir == true]" show={step >= 30} />

          <ActivationBox x={pos.admin} y={3360} height={100} show={step >= 30} />
          <ActivationBox x={pos.panelIU} y={3360} height={100} show={step >= 30} />
          <ActivationBox x={pos.panelCtrl} y={3370} height={80} show={step >= 30} />

          <Message fromX={pos.admin} toX={pos.panelIU} y={3370} label="30: salirGestionGlobal()" show={step >= 30} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={3390} label="30.1: salirGestionGlobal()" show={step >= 31} />
          <Message fromX={pos.panelCtrl} toX={pos.panelIU} y={3410} label="30.2: irAmbitoPrincipal()" isReturn={true} show={step >= 32} />
          <Message fromX={pos.panelIU} toX={pos.admin} y={3430} label="30.3: irAmbitoPrincipal()" isReturn={true} show={step >= 32} />

          {/* --- FINAL EXIT SEQUENCE (PASOS 33-35) --- */}
          <ActivationBox x={pos.admin} y={3600} height={100} show={step >= 33} />
          <ActivationBox x={pos.panelIU} y={3600} height={100} show={step >= 33} />
          <ActivationBox x={pos.panelCtrl} y={3610} height={80} show={step >= 33} />

          <Message fromX={pos.admin} toX={pos.panelIU} y={3610} label="31: exitGlobalScope()" show={step >= 33} />
          <Message fromX={pos.panelIU} toX={pos.panelCtrl} y={3630} label="31.1: closeContext()" show={step >= 34} />
          <Message fromX={pos.panelCtrl} toX={pos.admin} y={3660} label="31.2: return" isReturn={true} show={step >= 35} />
        </svg>
      </div>
    );
  };
    ...
    return (
      <div className="min-w-[1200px] p-8 flex justify-center items-start pt-16">
        <svg width="1200" height={svgHeight} viewBox={`0 0 1200 ${svgHeight}`} className="drop-shadow-2xl font-sans">
          ...
          {/* Continuous Activation Bars */}
          <ActivationBox x={pos.user} y={100} height={2650} show={step >= 0} />
          ...
          <ActivationBox x={pos.ui} y={320} height={200} show={step >= 5} />
          <ActivationBox x={pos.control} y={330} height={160} show={step >= 5} />
          ...
          <Message fromX={pos.ui} toX={pos.user} y={500} label="2.1.4.1.1: getNewListRecetas()" isReturn={true} show={step >= 9} />
    ...
    return (
      <div className="min-w-[1200px] p-8 flex justify-center items-start pt-16">
        <svg width="1200" height={svgHeight} viewBox={`0 0 1200 ${svgHeight}`} className="drop-shadow-2xl font-sans">
          ...
          {/* Continuous Activation Bars */}
          <ActivationBox x={pos.user} y={100} height={3050} show={step >= 0} />
          ...
          <ActivationBox x={pos.ui} y={970} height={1800} show={step >= 29} />
          <ActivationBox x={pos.control} y={980} height={1780} show={step >= 29} />
          ...
          <GroupFrame x={pos.user - 40} y={1030} width={1150} height={1000} type="alt" label="Valoraciones Detalladas" show={step >= 31} />

          {/* SABOR POSITIVA */}
          <ActivationBox x={pos.receta} y={1050} height={80} show={step >= 31} />
          <ActivationBox x={pos.valorar} y={1070} height={40} show={step >= 32} />
          <Message fromX={pos.control} toX={pos.receta} y={1060} label="9.1.1: guardarComentarioSabor()" show={step >= 31} />
          <Message fromX={pos.receta} toX={pos.valorar} y={1080} label="9.1.1.1: asignarPuntuacionSabor()" show={step >= 32} />
          <Message fromX={pos.valorar} toX={pos.receta} y={1100} label="9.1.1.2: return()" isReturn={true} show={step >= 32} />
          <Message fromX={pos.receta} toX={pos.control} y={1120} label="9.1.1.3: return()" isReturn={true} show={step >= 32} />

          {/* SABOR NEGATIVA */}
          <AltDivider x={pos.user - 40} y={1150} width={1150} label="[valoracionSaborNegativa]" show={step >= 33} />
          <ActivationBox x={pos.receta} y={1160} height={80} show={step >= 33} />
          <ActivationBox x={pos.valorar} y={1180} height={40} show={step >= 34} />
          <Message fromX={pos.control} toX={pos.receta} y={1170} label="9.1.2: guardarComentario()" show={step >= 33} />
          <Message fromX={pos.receta} toX={pos.valorar} y={1190} label="9.1.2.1: asignarPuntuacionSabor()" show={step >= 34} />
          <Message fromX={pos.valorar} toX={pos.receta} y={1210} label="9.1.2.2: return()" isReturn={true} show={step >= 34} />
          <Message fromX={pos.receta} toX={pos.control} y={1230} label="9.1.2.3: return()" isReturn={true} show={step >= 34} />

          <Message fromX={pos.control} toX={pos.ui} y={1260} label="9.1.3: formularioPropuesta()" isReturn={true} show={step >= 35} />
          <Message fromX={pos.ui} toX={pos.user} y={1280} label="9.1.3: formularioPropuesta()" isReturn={true} show={step >= 36} />
          <Message fromX={pos.user} toX={pos.ui} y={1300} label="10: guardarPropuesta()" show={step >= 37} />

          {/* DIFICULTAD POSITIVA */}
          <AltDivider x={pos.user - 40} y={1350} width={1150} label="[valoracionDificultadPositiva]" show={step >= 38} />
          <ActivationBox x={pos.receta} y={1360} height={80} show={step >= 38} />
          <ActivationBox x={pos.valorar} y={1380} height={40} show={step >= 39} />
          <Message fromX={pos.control} toX={pos.receta} y={1370} label="9.1.3: guardarComentarioDificultad()" show={step >= 38} />
          <Message fromX={pos.receta} toX={pos.valorar} y={1390} label="9.1.3.1: asignarPuntuacionDificultad()" show={step >= 39} />
          <Message fromX={pos.valorar} toX={pos.receta} y={1410} label="9.1.3.2: return()" isReturn={true} show={step >= 39} />
          <Message fromX={pos.receta} toX={pos.control} y={1430} label="9.1.3.3: return()" isReturn={true} show={step >= 39} />

          {/* DIFICULTAD NEGATIVA */}
          <AltDivider x={pos.user - 40} y={1460} width={1150} label="[valoracionDificultadNegativa]" show={step >= 40} />
          <ActivationBox x={pos.receta} y={1470} height={80} show={step >= 40} />
          <ActivationBox x={pos.valorar} y={1490} height={40} show={step >= 41} />
          <Message fromX={pos.control} toX={pos.receta} y={1480} label="9.1.5: guardarComentarioDificultad()" show={step >= 40} />
          <Message fromX={pos.receta} toX={pos.valorar} y={1500} label="9.1.5.1: asignarPuntuacionDificultad()" show={step >= 41} />
          <Message fromX={pos.valorar} toX={pos.receta} y={1520} label="9.1.5.2: return()" isReturn={true} show={step >= 41} />
          <Message fromX={pos.receta} toX={pos.control} y={1540} label="9.1.5.3: return()" isReturn={true} show={step >= 41} />

          <Message fromX={pos.control} toX={pos.ui} y={1570} label="9.1.4.1: formularioPropuesta()" isReturn={true} show={step >= 42} />
          <Message fromX={pos.ui} toX={pos.user} y={1590} label="9.1.4.1: formularioPropuesta()" isReturn={true} show={step >= 43} />
          <Message fromX={pos.user} toX={pos.ui} y={1610} label="11: guardarPropuesta()" show={step >= 44} />

          {/* TIEMPO POSITIVA */}
          <AltDivider x={pos.user - 40} y={1660} width={1150} label="[valoracionTiempoPositiva]" show={step >= 45} />
          <ActivationBox x={pos.receta} y={1670} height={80} show={step >= 45} />
          <ActivationBox x={pos.valorar} y={1690} height={40} show={step >= 46} />
          <Message fromX={pos.control} toX={pos.receta} y={1680} label="9.1.6: guardarComentarioTiempo()" show={step >= 45} />
          <Message fromX={pos.receta} toX={pos.valorar} y={1700} label="9.1.6.1: asignarPuntuacionTiempo()" show={step >= 46} />
          <Message fromX={pos.valorar} toX={pos.receta} y={1720} label="9.1.6.2: return()" isReturn={true} show={step >= 46} />
          <Message fromX={pos.receta} toX={pos.control} y={1740} label="9.1.6.3: return()" isReturn={true} show={step >= 46} />

          {/* TIEMPO NEGATIVA */}
          <AltDivider x={pos.user - 40} y={1770} width={1150} label="[valoracionTiempoNegativa]" show={step >= 47} />
          <ActivationBox x={pos.receta} y={1780} height={80} show={step >= 47} />
          <ActivationBox x={pos.valorar} y={1800} height={40} show={step >= 48} />
          <Message fromX={pos.control} toX={pos.receta} y={1790} label="9.1.7: guardarComentarioTiempo()" show={step >= 47} />
          <Message fromX={pos.receta} toX={pos.valorar} y={1810} label="9.1.7.1: asignarPuntuacionTiempo()" show={step >= 48} />
          <Message fromX={pos.valorar} toX={pos.receta} y={1830} label="9.1.7.2: return()" isReturn={true} show={step >= 48} />
          <Message fromX={pos.receta} toX={pos.control} y={1850} label="9.1.7.3: return()" isReturn={true} show={step >= 48} />

          <Message fromX={pos.control} toX={pos.ui} y={1880} label="9.1.8.1: formularioPropuesta()" isReturn={true} show={step >= 49} />
          <Message fromX={pos.ui} toX={pos.user} y={1900} label="9.1.8.1: formularioPropuesta()" isReturn={true} show={step >= 50} />
          <Message fromX={pos.user} toX={pos.ui} y={1920} label="12: guardarPropuesta()" show={step >= 51} />

          {/* --- COMENTARIOS Y EDICIÓN --- */}
          <GroupFrame x={pos.user - 40} y={2050} width={1150} height={250} type="alt" label="[ClickOn_comentar == true]" show={step >= 52} />
          <ActivationBox x={pos.ui} y={2070} height={160} show={step >= 52} />
          <ActivationBox x={pos.control} y={2080} height={140} show={step >= 52} />
          <ActivationBox x={pos.receta} y={2110} height={80} show={step >= 54} />
          <ActivationBox x={pos.valorar} y={2130} height={40} show={step >= 55} />

          <Message fromX={pos.user} toX={pos.ui} y={2080} label="13: Comentar()" show={step >= 52} />
          <Message fromX={pos.ui} toX={pos.control} y={2100} label="13.1: AñadirComentario()" show={step >= 53} />
          <Message fromX={pos.control} toX={pos.receta} y={2120} label="13.1.1: CargarReceta()" show={step >= 54} />
          <Message fromX={pos.receta} toX={pos.valorar} y={2140} label="13.1.1.1: RealizarComentario()" show={step >= 55} />

          <Message fromX={pos.valorar} toX={pos.receta} y={2160} label="13.1.1.2: return()" isReturn={true} show={step >= 55} />
          <Message fromX={pos.receta} toX={pos.control} y={2180} label="13.1.2: return()" isReturn={true} show={step >= 55} />
          <Message fromX={pos.control} toX={pos.ui} y={2200} label="13.2: return()" isReturn={true} show={step >= 55} />
          <Message fromX={pos.ui} toX={pos.user} y={2220} label="13.3: return()" isReturn={true} show={step >= 55} />

          {/* EDICION */}
          <AltDivider x={pos.user - 40} y={2320} width={1150} label="[ClickOn_modificarComentario == true]" show={step >= 56} />
          <ActivationBox x={pos.ui} y={2340} height={180} show={step >= 56} />
          <ActivationBox x={pos.control} y={2350} height={160} show={step >= 56} />
          <ActivationBox x={pos.receta} y={2370} height={80} show={step >= 58} />
          <ActivationBox x={pos.valorar} y={2390} height={40} show={step >= 59} />

          <Message fromX={pos.user} toX={pos.ui} y={2350} label="14: ModificarComentario()" show={step >= 56} />
          <Message fromX={pos.ui} toX={pos.control} y={2360} label="14.1: ModificarComentario()" show={step >= 57} />
          <Message fromX={pos.control} toX={pos.receta} y={2370} label="14.1.1: CargarReceta()" show={step >= 58} />
          <Message fromX={pos.receta} toX={pos.valorar} y={2390} label="14.1.1.1: RealizarModificación()" show={step >= 59} />

          <Message fromX={pos.valorar} toX={pos.receta} y={2410} label="14.1.1.1.1: DevolverModificacion()" isReturn={true} show={step >= 60} />
          <Message fromX={pos.receta} toX={pos.control} y={2430} label="14.1.2: MarcarEditado()" isReturn={true} show={step >= 60} />
          <Message fromX={pos.control} toX={pos.ui} y={2450} label="14.2: MostrarEditado()" isReturn={true} show={step >= 61} />
          <Message fromX={pos.ui} toX={pos.user} y={2470} label="14.2: MostrarEditado()" isReturn={true} show={step >= 62} />

          {/* --- RECOMENDACIONES (IA/PATRONES) --- */}
          <GroupFrame x={pos.user - 40} y={2550} width={1150} height={300} type="alt" label="[PatronDeGustos == true]" show={step >= 63} />
          <ActivationBox x={pos.ui} y={2580} height={60} show={step >= 63} />
          <ActivationBox x={pos.control} y={2590} height={40} show={step >= 63} />
          <ActivationBox x={pos.receta} y={2600} height={30} show={step >= 63} />
          <ActivationBox x={pos.valorar} y={2610} height={30} show={step >= 63} />
          <ActivationBox x={pos.favoritos} y={2610} height={30} show={step >= 63} />

          <Message fromX={pos.favoritos} toX={pos.receta} y={2610} label="16: patronGustos()" show={step >= 64} />
          <Message fromX={pos.receta} toX={pos.control} y={2620} label="15: sugerirRecetasPersonalizadas()" isReturn={true} show={step >= 65} />
          <Message fromX={pos.control} toX={pos.ui} y={2640} label="15.1: sugerirRecetasPersonalizadas()" isReturn={true} show={step >= 66} />
          <Message fromX={pos.ui} toX={pos.user} y={2660} label="15.1.1: sugerirRecetasPersonalizadas()" isReturn={true} show={step >= 67} />

          {/* SIN PATRON */}
          <AltDivider x={pos.user - 40} y={2710} width={1150} label="[PatronDeGustos == False]" show={step >= 68} />
          <ActivationBox x={pos.ui} y={2730} height={60} show={step >= 69} />
          <ActivationBox x={pos.control} y={2740} height={40} show={step >= 69} />
          <ActivationBox x={pos.receta} y={2750} height={30} show={step >= 69} />
          <ActivationBox x={pos.valorar} y={2760} height={30} show={step >= 69} />

          <Message fromX={pos.valorar} toX={pos.receta} y={2760} label="17: patronValoraciones()" show={step >= 70} />
          <Message fromX={pos.receta} toX={pos.control} y={2770} label="17.1: sugerirRecetas()" isReturn={true} show={step >= 71} />
          <Message fromX={pos.control} toX={pos.ui} y={2790} label="17.1.1: sugerirRecetas()" isReturn={true} show={step >= 72} />
          <Message fromX={pos.ui} toX={pos.user} y={2810} label="17.1.1.1: sugerirRecetas()" isReturn={true} show={step >= 73} />
  ...
  const renderWeeklyPlanning = () => {
    // DIAGRAMA DE PLANIFICACIÓN SEMANAL (REVISADO)
    const pos = {
      user: 60,
      ui: 250,
      control: 450,
    ...
    return (
      <div className="min-w-[1100px] p-8 flex justify-center items-start pt-16">
        <svg width="1100" height={svgHeight} viewBox={`0 0 1100 ${svgHeight}`} className="drop-shadow-2xl font-sans">
          ...
          <g className={`transition-opacity duration-1000 ${step >= 0 ? 'opacity-100' : 'opacity-0'}`}>
            <Lifeline x={pos.user} height={svgHeight - 50} label="usuario" type="actor" color="text-slate-300" />
            <Lifeline x={pos.ui} height={svgHeight - 50} label="Planificacion semanal IU" subLabel="UI" type="boundary" icon={Layout} color="text-blue-400" />
            <Lifeline x={pos.control} height={svgHeight - 50} label="Planificacion semanal" type="control" icon={RefreshCw} color="text-blue-400" />
            <Lifeline x={pos.recetas} height={svgHeight - 50} label="M :Recetas" type="entity" icon={BookOpen} color="text-sky-300" />
            <Lifeline x={pos.lista} height={svgHeight - 50} label="M:lista de la compra" type="entity" icon={ShoppingCart} color="text-sky-300" />
            <Lifeline x={pos.planificacion} height={svgHeight - 50} label="M :planificacion" type="entity" icon={ClipboardList} color="text-sky-300" />
          </g>

          {/* Continuous Activation Bars */}
          <ActivationBox x={pos.user} y={100} height={2050} show={step >= 0} />

          {/* --- BLOQUE: LOGIN (Pasos 1-4) --- */}
          <GroupFrame x={pos.user - 40} y={100} width={pos.control - pos.user + 50} height={200} type="alt" label="[Login == True]" show={step >= 1} />

          <ActivationBox x={pos.ui} y={130} height={120} show={step >= 1} />
          <ActivationBox x={pos.control} y={140} height={100} show={step >= 1} />

          <Message fromX={pos.user} toX={pos.ui} y={140} label="1: AccederPlanificacion()" show={step >= 2} />
          <Message fromX={pos.ui} toX={pos.control} y={150} label="1.1: AccederPlanificacion()" show={step >= 3} />
          <Message fromX={pos.control} toX={pos.ui} y={180} label="1.2: mostrarAmbitoPlanificacion()" isReturn={true} show={step >= 4} />
          <Message fromX={pos.ui} toX={pos.user} y={200} label="1.3: mostrarAmbitoPlanificacion()" isReturn={true} show={step >= 5} />

          {/* --- BLOQUE: CONFIRMACION KO (Pasos 5-21) --- */}
          {/* Main Frame for the whole lower section */}
          <GroupFrame x={pos.user - 50} y={320} width={1120} height={2300} type="alt" label="[confirmacionKO]" show={step >= 6} />
          <text x={pos.user - 45} y={345} fontSize="10" fill="#64748b">[ClickOn_diasEspecificos]</text>

          {/* --- PREFERENCIAS KO --- */}
          <GroupFrame x={pos.user - 40} y={360} width={1080} height={150} type="alt" label="[PreferenciasKO]" show={step >= 7} />

          <ActivationBox x={pos.ui} y={380} height={100} show={step >= 8} />
          <ActivationBox x={pos.control} y={390} height={80} show={step >= 8} />
          <ActivationBox x={pos.recetas} y={400} height={60} show={step >= 10} />
          <ActivationBox x={pos.lista} y={410} height={30} show={step >= 11} />
          <ActivationBox x={pos.planificacion} y={420} height={30} show={step >= 12} />

          <Message fromX={pos.user} toX={pos.ui} y={390} label="2: defConfig()" show={step >= 8} />
          <Message fromX={pos.ui} toX={pos.control} y={400} label="2.1: defConfig()" show={step >= 9} />
          <Message fromX={pos.control} toX={pos.recetas} y={410} label="3: elegirRecetas()" show={step >= 10} />
          <Message fromX={pos.recetas} toX={pos.lista} y={420} label="3.1: addIngredientes()" show={step >= 11} />
          <Message fromX={pos.recetas} toX={pos.planificacion} y={440} label="3.2: planificarRecetas()" isReturn={true} show={step >= 12} />

          {/* --- PREFERENCIAS OK --- */}
          <AltDivider x={pos.user - 40} y={530} width={1080} label="[PreferenciasOK]" show={step >= 13} />

          {/* Loop */}
          <GroupFrame x={pos.user - 30} y={560} width={1050} height={300} type="loop" label="loop (0, dias)" show={step >= 14} />
          <text x={pos.user - 20} y={585} fontSize="10" fill="#64748b">[Hasta completar dias de la semana]</text>

          <ActivationBox x={pos.ui} y={590} height={200} show={step >= 15} />
          <ActivationBox x={pos.control} y={600} height={180} show={step >= 15} />
          <ActivationBox x={pos.recetas} y={640} height={100} show={step >= 19} />
          <ActivationBox x={pos.lista} y={660} height={40} show={step >= 20} />
          <ActivationBox x={pos.planificacion} y={680} height={40} show={step >= 21} />

          <Message fromX={pos.user} toX={pos.ui} y={610} label="4: elegirPreferencias()" show={step >= 15} />
          <Message fromX={pos.ui} toX={pos.control} y={620} label="4.1: elegirPreferencias()" show={step >= 16} />

          <Message fromX={pos.user} toX={pos.ui} y={650} label="6: elegirRestricciones()" show={step >= 17} />
          <Message fromX={pos.ui} toX={pos.control} y={660} label="4.2: elegirRestricciones()" show={step >= 18} />

          <Message fromX={pos.control} toX={pos.recetas} y={670} label="4.1.1: elegirRecetas()" show={step >= 19} />
          <Message fromX={pos.recetas} toX={pos.lista} y={680} label="5: addIngredientes()" show={step >= 20} />
          <Message fromX={pos.recetas} toX={pos.planificacion} y={700} label="4.1.1.1: planificarRecetas()" isReturn={true} show={step >= 21} />

          {/* --- BLOCK: SEMANA COMPLETA (steps 22+) --- */}
          <AltDivider x={pos.user - 50} y={900} width={1120} label="[ClickOn_semanaCompleta]" show={step >= 22} />

          {/* --- PREFERENCIAS KO (Semana Completa) --- */}
          <GroupFrame x={pos.user - 40} y={930} width={1080} height={150} type="alt" label="[PreferenciasKO]" show={step >= 23} />

          <ActivationBox x={pos.ui} y={950} height={100} show={step >= 23} />
          <ActivationBox x={pos.control} y={960} height={80} show={step >= 23} />
          <ActivationBox x={pos.recetas} y={970} height={60} show={step >= 25} />
          <ActivationBox x={pos.lista} y={980} height={30} show={step >= 26} />
          <ActivationBox x={pos.planificacion} y={990} height={30} show={step >= 27} />

          <Message fromX={pos.user} toX={pos.ui} y={960} label="9: defConfig()" show={step >= 23} />
          <Message fromX={pos.ui} toX={pos.control} y={970} label="8: defConfig()" show={step >= 24} />
          <Message fromX={pos.control} toX={pos.recetas} y={980} label="7: elegirRecetas()" show={step >= 25} />
          <Message fromX={pos.recetas} toX={pos.lista} y={990} label="7.1: addIngredientes()" show={step >= 26} />
          <Message fromX={pos.recetas} toX={pos.planificacion} y={1010} label="7.2: planificarRecetas()" isReturn={true} show={step >= 27} />

          {/* --- PREFERENCIAS OK (Semana Completa) --- */}
          <AltDivider x={pos.user - 40} y={1100} width={1080} label="[PreferenciasOK]" show={step >= 28} />

          {/* Loop */}
          <GroupFrame x={pos.user - 30} y={1130} width={1050} height={300} type="loop" label="loop (0, 7)" show={step >= 29} />
          <text x={pos.user - 20} y={1155} fontSize="10" fill="#64748b">[Hasta completar dias de la semana]</text>

          <ActivationBox x={pos.ui} y={1160} height={200} show={step >= 30} />
          <ActivationBox x={pos.control} y={1170} height={180} show={step >= 30} />
          <ActivationBox x={pos.recetas} y={1210} height={100} show={step >= 34} />
          <ActivationBox x={pos.lista} y={1220} height={40} show={step >= 35} />
          <ActivationBox x={pos.planificacion} y={1230} height={40} show={step >= 36} />

          <Message fromX={pos.user} toX={pos.ui} y={1180} label="10: elegirPreferencias()" show={step >= 30} />
          <Message fromX={pos.ui} toX={pos.control} y={1190} label="10.1: elegirPreferencias()" show={step >= 31} />

          <Message fromX={pos.user} toX={pos.ui} y={1220} label="12: elegirRestricciones()" show={step >= 32} />
          <Message fromX={pos.ui} toX={pos.control} y={1230} label="10.2: elegirRestricciones()" show={step >= 33} />

          <Message fromX={pos.control} toX={pos.recetas} y={1240} label="10.1.1: elegirRecetas()" show={step >= 34} />
          <Message fromX={pos.recetas} toX={pos.lista} y={1250} label="11: addIngredientes()" show={step >= 35} />
          <Message fromX={pos.recetas} toX={pos.planificacion} y={1270} label="10.1.1.1: planificarRecetas()" isReturn={true} show={step >= 36} />

          {/* --- REVISAR PLANIFICACION (Steps 37+) --- */}
          <ActivationBox x={pos.ui} y={1600} height={150} show={step >= 37} />
          <ActivationBox x={pos.control} y={1610} height={120} show={step >= 37} />
          <ActivationBox x={pos.planificacion} y={1620} height={60} show={step >= 39} />

          <Message fromX={pos.user} toX={pos.ui} y={1610} label="13: revisarPlanificacion()" show={step >= 37} />
          <Message fromX={pos.ui} toX={pos.control} y={1620} label="13.1: revisarPlanificacion()" show={step >= 38} />
          <Message fromX={pos.control} toX={pos.planificacion} y={1630} label="13.1.1: revisarPlanificacion()" show={step >= 39} />

          <Message fromX={pos.planificacion} toX={pos.control} y={1650} label="13.1.2: mostrarPlanificacion()" isReturn={true} show={step >= 40} />
          <Message fromX={pos.control} toX={pos.ui} y={1670} label="14: mostrarPlanificacion()" isReturn={true} show={step >= 41} />
          <Message fromX={pos.ui} toX={pos.user} y={1690} label="13.2: mostrarPlanificacion()" isReturn={true} show={step >= 42} />

          {/* --- CONFIRMACION OK (Steps 43+) --- */}
          <AltDivider x={pos.user - 50} y={1730} width={1120} label="[ConfirmacionOK]" show={step >= 43} />

          <ActivationBox x={pos.ui} y={1760} height={500} show={step >= 44} />
          <ActivationBox x={pos.control} y={1770} height={480} show={step >= 44} />
          <ActivationBox x={pos.planificacion} y={1780} height={460} show={step >= 46} />
          <ActivationBox x={pos.lista} y={1800} height={40} show={step >= 47} />

          <Message fromX={pos.user} toX={pos.ui} y={1770} label="15: planificacionOk()" show={step >= 44} />
          <Message fromX={pos.ui} toX={pos.control} y={1780} label="15.1: planificacionOk()" show={step >= 45} />

          <Message fromX={pos.control} toX={pos.planificacion} y={1790} label="15.1.1: guardarPlanificacion()" show={step >= 46} />
          <Message fromX={pos.planificacion} toX={pos.lista} y={1810} label="15.1.1.1: genList()" show={step >= 47} />

          {/* DASHED RETURNS */}
          {/* mostrarListaCompra */}
          <Message fromX={pos.planificacion} toX={pos.control} y={1840} label="15.1.1.1.1: mostrarListaCompra()" isReturn={true} show={step >= 48} />
          <Message fromX={pos.control} toX={pos.ui} y={1860} label="15.1.1.1.1: mostrarListaCompra()" isReturn={true} show={step >= 49} />
          <Message fromX={pos.ui} toX={pos.user} y={1880} label="15.1.1.1.1: mostrarListaCompra()" isReturn={true} show={step >= 50} />

          {/* sincronizarCalendario */}
          <Message fromX={pos.planificacion} toX={pos.control} y={1920} label="15.1.1.2: sincronizarCalendario()" isReturn={true} show={step >= 51} />
          <Message fromX={pos.control} toX={pos.ui} y={1940} label="15.1.1.2.1: sincronizarCalendario()" isReturn={true} show={step >= 52} />
          <Message fromX={pos.ui} toX={pos.user} y={1960} label="15.1.1.2.1.1: sincronizarCalendario()" isReturn={true} show={step >= 53} />

          {/* addNotificaciones */}
          <Message fromX={pos.planificacion} toX={pos.control} y={2000} label="15.1.1.3: addNotificaciones()" isReturn={true} show={step >= 54} />
          <Message fromX={pos.control} toX={pos.ui} y={2020} label="15.1.1.3.1: addNotificaciones()" isReturn={true} show={step >= 55} />
          <Message fromX={pos.ui} toX={pos.user} y={2040} label="15.1.1.3.1.1: addNotificaciones()" isReturn={true} show={step >= 56} />
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-white overflow-hidden font-sans selection:bg-blue-500 selection:text-white relative">
            ...
            <nav className="flex-1 space-y-2 overflow-y-auto">
              {diagramsList.map(diagram => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg transition-all duration-200 flex items-center gap-3 ${activeDiagram.id === diagram.id
      ...
      <div className="flex justify-between items-center px-6 py-3 bg-slate-900 border-b border-slate-800 shadow-xl z-20 relative">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex bg-slate-800 rounded-lg p-1 border border-slate-700">
            <span className="text-sm font-mono text-slate-400 bg-slate-800 px-2 py-1 rounded border border-slate-700">
              {globalStep === totalSteps ? 'FINAL' : `Paso ${globalStep} / ${totalSteps}`}
            </span>
            <button onClick={reset} className="p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-white" title="Reiniciar">
              <RefreshCw size={18} />
            </button>
            <div className="flex bg-slate-800 rounded-lg p-1 border border-slate-700">
              <button onClick={prevStep} className="p-2 hover:bg-slate-700 rounded transition-colors text-slate-300" disabled={globalStep === 0}>
                <ArrowLeft size={20} />
              </button>
              <button onClick={nextStep} className="p-2 hover:bg-blue-600 rounded transition-colors bg-blue-700 text-white shadow-lg ml-1" disabled={globalStep === totalSteps}>
                {globalStep === totalSteps ? <Globe size={20} /> : (globalStep === 0 ? <Play size={20} fill="currentColor" /> : <ArrowRight size={20} />)}
              </button>
            </div>
          </div>
        </div>
      ...
      >
        ...
        {activeDiagram.id === 'home' && renderHome()}

        {/* LOGIC FOR DIAGRAMS WITH IMAGES */}
        {hasImages && globalStep < 3 ? (
          renderIntroImages()
        ) : (
          <>
            {activeDiagram.id === 'main-sequence' && (
              globalStep === totalSteps ? renderWebEmbed() : renderMainSequence()
            )}

            {activeDiagram.id === 'global-user-mgmt' && (
              globalStep === totalSteps ? renderWebEmbed() : renderGlobalUserManagement()
            )}

            {activeDiagram.id === 'recipe-mgmt' && (
              globalStep === totalSteps ? renderWebEmbed() : renderRecipeManagement()
            )}

            {activeDiagram.id === 'search-discover' && (
              globalStep === totalSteps ? renderWebEmbed() : renderSearchAndDiscover()
            )}
        ...
        {activeDiagram.id === 'acknowledgments' && renderAcknowledgments()}
